...
if ($versionNeedsRefresh) {
    ...
    try {
        $vcResponse = Invoke-RestMethod -Uri "https://api.github.com/repos/haukurk/claude-status-line/releases/latest" `
...
if ($versionData) {
    try {
        ...
        if ($latestTag -and (Test-VersionGreaterThan $latestTag $VERSION)) {
            $updateLine = "`n${dim}Update available: ${latestTag} → https://github.com/haukurk/claude-status-line${reset}"
        }
    } catch {}
}
